...
class MarketDataAgent:
    def __init__(self, db_config, redis_config, mt5_config=None, mode='simulate', regime_model='comprehensive'):
        ...
        # Rolling window for feature generation
        self.latest_data = {}
        self.window_data = {}
        self.window_size = 1000   # enough for M1/M2
    ...
    def ingest_data(self, data):
        try:
            ...
            self.latest_data[normalized['symbol']] = normalized
            if normalized['symbol'] not in self.window_data:
                self.window_data[normalized['symbol']] = []
            window = self.window_data[normalized['symbol']]
            ...
            regime_features, nnfx_signals = self.feature_store.compute_unified_regime_features(
                normalized['symbol'], normalized, historical_data
            )
            
            self.feature_store.store_ml_features(normalized['symbol'], regime_features, regime_features.regime_type, nnfx_signals)
            self.feature_store.store_strategy_features(normalized['symbol'], regime_features, regime_features.regime_type, nnfx_signals)
            self.feature_store.store_regime_history(normalized['symbol'], regime_features.regime_type, regime_features.regime_confidence, nnfx_signals)
            self.feature_store.store_ml_training_features(normalized['symbol'], regime_features, nnfx_signals, normalized, datetime.now())
            ...
            self.event_store.append_event('data_ingested_with_regime', normalized['symbol'],
                {'open': normalized.get('open'), 'close': normalized.get('close'),
                 'regime': regime_features.regime_type, 'confidence': regime_features.regime_confidence,
                 'enhanced_confidence': regime_features.regime_confidence_enhanced,
                 'volatility': regime_features.volatility, 'trend_strength': regime_features.trend_strength,
                 'nnfx_signal': nnfx_signals.nnfx_signal, 'signal_confidence': nnfx_signals.signal_confidence},
                metadata={'source': self.mode, 'regime_model': self.regime_model,
                          'regime_features': {'volatility': regime_features.volatility,
                                              'trend_strength': regime_features.trend_strength,
                                              'volume_profile': regime_features.volume_profile,
                                              'adx': regime_features.adx, 'atr': regime_features.atr,
                                              'rsi': regime_features.rsi}})
            
            logger.info(f"Ingested {normalized['symbol']} | Regime: {regime_features.regime_type} | "
                       f"Confidence: {regime_features.regime_confidence:.3f} | NNFX: {nnfx_signals.nnfx_signal}")
        except Exception as e:
            logger.error(f"Error ingesting data: {e}")
            raise

    def get_window_df(self, symbol: str):
        if symbol not in self.window_data:
            return pd.DataFrame()
        df = pd.DataFrame(self.window_data[symbol])
    ...
    def run_historical(self, days=30, timeframe="H1"):
        ...
        for symbol in self.symbols:
            if not self._enable_symbol(symbol):
                continue
            try:
                rates = self._fetch_historical(symbol, self.timeframes[timeframe], start_date, datetime.now())
                if rates is not None:
                    for rate in rates:
                        data = {'time': datetime.fromtimestamp(rate[0]), 'symbol': symbol,
                                'open': rate[1], 'high': rate[2], 'low': rate[3],
                                'close': rate[4], 'volume': rate[5] if len(rate) > 5 else 1000}
        ...
        while self.running:
            for symbol in self.symbols:
                try:
                    ...
                    if tick is None:
                        continue
                    data = {'time': datetime.now(), 'symbol': symbol, 'open': tick.bid,
                            'high': tick.bid, 'low': tick.bid, 'close': tick.bid, 'volume': 1000}
                    self.ingest_data(data)
                except Exception as e:
    ...
    def run_simulate(self):
        logger.info("Starting simulation data generation (M1 interval)")
        if not self.running:
            self.running = True
