...
class BacktestAgent(BaseAgent):
    ...
    def _compute_report(self, returns: list, total_trades: int) -> BacktestReport:
        returns = np.array(returns)
        wins = (returns > 0).sum()
        losses = (returns <= 0).sum()
        win_rate = wins / total_trades if total_trades else 0.0
        avg_ret = returns.mean()
        sharpe = avg_ret / returns.std() * np.sqrt(252) if returns.std() > 0 else 0.0
        cum = np.cumsum(returns)
        running_max = np.maximum.accumulate(cum)
        drawdowns = cum - running_max
        max_dd = abs(drawdowns.min()) if len(drawdowns) else 0.0
        ...
        return BacktestReport(
            ...
            feature_consistency_score=1.0,
            regime_correctness_score=1.0,
        )